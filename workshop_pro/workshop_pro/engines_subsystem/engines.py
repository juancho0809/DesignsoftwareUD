...
class Engine:
    ...
    def is_in_speed(self, min_speed: int, max_speed: int) -> bool:
        ...
        return min_speed <= self.maximum_speed <= max_speed

    def __str__(self) -> str:
        return f"Engine: torque={self.torque}, \
                maximum_speed={self.maximum_speed}, \
                dimenssions={self.dimenssions}, \
                power={self.power}, \
...
class HybridEngine(Engine):
    ...
    def __init__(
        ...
        stability: str,
        weight: float,
        electric_power: int,  # Add electric_power parameter (int)
    ):
